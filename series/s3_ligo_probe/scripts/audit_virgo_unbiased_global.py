...
def audit_unbiased_global_floor():
    print(f"[*] LYRA DIRECTIVE: Calculating TRUE UNBIASED Global Virgo Coherence Floor...")
    
    events = {}
    for f in glob.glob(os.path.join(DATA_DIR, "*.hdf5")):
        base = os.path.basename(f)
        
        # [LYRA'S CRITICAL FIX]: Split from the RIGHT to handle "GW190412_053044_H1.hdf5"
    ...
    for event, files in three_det_events.items():
        try:
            # Pre-Merger Baseline ONLY. Totally blind to merger success/failure.
    ...
    if not global_h1_v1_coherence:
        print("[!] ERROR: No coherence data processed. Check file parsing.")
        return
    ...
    print("==================================================")
    print("--- UNBIASED VIRGO EMPIRICAL NOISE FLOOR (2.8k - 5.5k Hz) ---")
    print("==================================================")
    ...
    print("-" * 50)
    print(f"[+] TRUE UNBIASED VIRGO THRESHOLD: > {derived_threshold:.4f}")
    print("==================================================")

if __name__ == "__main__":
    audit_unbiased_global_floor()